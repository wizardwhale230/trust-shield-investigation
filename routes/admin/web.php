...
use App\Http\Controllers\Admin\ManageCasesController;
...
Route::middleware(['isadmin', '2fa'])->prefix('admin')->group(function () {
    ...
    Route::controller(AdminTicketController::class)->group(function () {
        Route::get('dashboard/support-tickets', 'index')->name('admin.support-tickets.index');
        Route::get('dashboard/support-tickets/{id}', 'show')->name('admin.support-tickets.show');
        Route::post('dashboard/support-tickets/{id}/reply', 'reply')->name('admin.support-tickets.reply');
        Route::put('dashboard/support-tickets/{id}/status', 'updateStatus')->name('admin.support-tickets.status');
    });

    // Case Management
    Route::controller(ManageCasesController::class)->group(function () {
        Route::get('dashboard/cases', 'index')->name('admin.cases');
        Route::get('dashboard/cases/{id}', 'show')->name('admin.cases.show');
        Route::post('dashboard/cases/{id}/assign', 'assign')->name('admin.cases.assign');
        Route::put('dashboard/cases/{id}/status', 'updateStatus')->name('admin.cases.status');
        Route::post('dashboard/cases/{id}/notes', 'addNote')->name('admin.cases.addnote');
        Route::post('dashboard/cases/{id}/recovery', 'creditRecovery')->name('admin.cases.recovery');
        Route::get('dashboard/cases/{id}/fee-request', 'createFeeRequest')->name('admin.cases.fee.create');
        Route::post('dashboard/fee-requests', 'storeFeeRequest')->name('admin.fee.store');
        Route::delete('dashboard/fee-requests/{id}', 'cancelFeeRequest')->name('admin.fee.cancel');
        Route::get('dashboard/cases/{id}/documents/{document}/download', 'downloadDocument')->name('admin.cases.document.download');
    });

    // Team Members
    Route::resource('admin/dashboard/team-members', TeamMemberController::class, [
        'names' => 'admin.team-members',
    ])->except(['show']);
