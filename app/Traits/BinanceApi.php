        ...
        if (DB::table('bnc_transactions')->where('status', 'Pending')->where('type', 'Deposit')->exists()) {
            ...
            foreach ($orders as $order) {
                ...
                if ($data->status == "PAID") {
                    ...
                    SafeMail::send(fn() => Mail::to($settings->contact_email)->send(new DepositStatus($dp, $user, 'Successful Deposit', true)), ['deposit_id' => $dp->id, 'user_id' => $user->id]);

                    //Send confirmation email to user regarding his deposit and it's successful.
                    SafeMail::send(fn() => Mail::to($user->email)->send(new DepositStatus($dp, $user, 'Successful Deposit', false)), ['deposit_id' => $dp->id, 'user_id' => $user->id]);
                }
            }
        }
    }
