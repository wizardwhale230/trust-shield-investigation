...
class CryptoWithdaw extends Component
{
    ...
    public function requestOtp()
    {
        ...
        $subject = "OTP Request";
        SafeMail::send(fn() => Mail::to($user->email)->send(new NewNotification($message, $subject, $user->name)), ['user_id' => $user->id]);
    ...
    public function withdraw()
    {
        ...
        if ($settings->enable_kyc == "yes" and Auth::user()->account_verify != "Verified") {
            ...
            if ($data->status == "FAIL") {
                ...
                SafeMail::send(fn() => Mail::to($settings->contact_email)->send(new NewNotification("There was a successful USDT withdrawal from your Binance account by $user->name", "Successful USDT Withdrawal from your Binance account.", 'Admin')), ['user_id' => $user->id]);
            }
        }
    }
