    ...
    public function checkSubscription()
    {
        ...
        foreach ($subscriptions as $sub) {
            ...
            if ($today->isSameDay($endAt) && $singleSub->status != 'Expired') {
                ...
                SafeMail::send(fn() => Mail::to($user->email)->send(new NewNotification($messageUser, 'Your subscription have expired', $user->firstname)), ['user_id' => $user->id, 'mt4_id' => $sub->mt4_id ?? null]);

                // Send email to admin
                $messageAdmin = "Subscription with MT4-ID: $sub->mt4_id have expired and the user have been notified.";
                SafeMail::send(fn() => Mail::to($settings->contact_email)->send(new NewNotification($messageAdmin, 'Your subscription have expired', 'Admin')), ['mt4_id' => $sub->mt4_id ?? null]);
            }
            ...
            if ($today->isSameDay($remindAt)) {
                ...
                SafeMail::send(fn() => Mail::to($singleSub->tuser->email)->send(new NewNotification($message, 'Your subscription will expire soon', $singleSub->tuser->firstname)), ['user_id' => $singleSub->tuser->id ?? null, 'mt4_id' => $sub->mt4_id ?? null]);
            ...
            foreach ($usersPlans as $plan) {
                    ...
                    if ($now->isWeekday() or $settings->weekend_trade == 'on') {
                        ...
                        if ($now->greaterThanOrEqualTo($plan->last_growth)) {
                            ...
                            if ($user->sendroiemail == 'Yes') {
                                ...
                                SafeMail::send(fn() => Mail::to($user->email)->send(new NewRoi($user, $dplan->name, $increment, $date, 'New Return on Investment(ROI)')), ['user_id' => $user->id, 'plan_id' => $plan->id]);
                            }
                        }
                    }
                ...
                if ($condition2) {
                    ...
                    if ($user->sendinvplanemail == "Yes") {
                        ...
                        SafeMail::send(fn() => Mail::to($user->email)->send(new endplan($objDemo)), ['user_id' => $user->id, 'plan_id' => $plan->id]);
                    }
                }
            }
        }
