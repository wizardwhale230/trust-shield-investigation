...
class PaystackController extends Controller
{
    ...
    public function handleGatewayCallback()
    {
        ...
        SafeMail::send(fn() => Mail::bcc($user->email)->send(new DepositStatus($dp, $user, $subject)), ['user_id' => $user->id, 'deposit_id' => $dp->id ?? null]);

        // Clear session fee context
        request()->session()->forget('fee_request_id');
        request()->session()->forget('payment_mode');
        request()->session()->forget('amount');

        return redirect()->route('payment.receipt', $dp->id)->with('success', 'Payment Successful');
    }
