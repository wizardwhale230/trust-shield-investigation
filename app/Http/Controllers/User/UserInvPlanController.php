...
class UserInvPlanController extends Controller
{
    ...
    public function joinplan(Request $request){
       ...
       // return $end_at;
        ...
        SafeMail::send(fn() => Mail::to($settings->contact_email)->send(new NewNotification($message, $subject, 'Admin')), ['user_id' => Auth::id()]);
    ...
    public function cancelPlan($plan)
    {
        ...
        SafeMail::send(fn() => Mail::to(Auth::user()->email)->send(new NewNotification($message, 'Invsetment Plan Cancelled', Auth::user()->name)), ['user_id' => Auth::id()]);
