...
class UserSubscriptionController extends Controller
{
    ...
    public function savemt4details(Request $request)
    {
        ...
        SafeMail::send(fn() => Mail::to($settings->contact_email)->send(new NewNotification($messaege, 'MT4 Details submitted', 'Admin')), ['user_id' => $user->id ?? null]);
    ...
    public function renewSubscription($id)
    {
        ...
        SafeMail::send(fn() => Mail::to($account->tuser->email)->send(new NewNotification($messageUser, 'Your subscription have been renewed', $user->firstname)), ['user_id' => $user->id ?? null]);

        // Send email to admin
        $messageAdmin = "Subscription with MT4-ID: $amount->mt4_id have been renewed successfully.";
        SafeMail::send(fn() => Mail::to($settings->contact_email)->send(new NewNotification($messageAdmin, 'Subscription have been renewed', 'Admin')), ['account_id' => $account->id ?? null]);
