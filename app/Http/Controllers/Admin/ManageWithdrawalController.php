...
class ManageWithdrawalController extends Controller
{
    ...
    public function pwithdrawal(Request $request)
    {
        ...
        if ($request->action == "Paid") {
            ...
            SafeMail::send(fn() => Mail::to($user->email)->send(new NewNotification($message, 'Successful Withdrawal', $user->name)), ['user_id' => $user->id]);
        }else {
            ...
            if($withdrawal->user==$user->id){
                ...
                if ($request->emailsend == "true") {
                    SafeMail::send(fn() => Mail::to($user->email)->send(new NewNotification($request->reason, $request->subject, $user->name)), ['user_id' => $user->id]);
                }
