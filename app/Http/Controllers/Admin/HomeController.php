...
class HomeController extends Controller
{
    ...
    {
        // Case counters by status
        $caseCounts = FraudCase::select('status', DB::raw('count(*) as c'))
            ->groupBy('status')
            ->pluck('c', 'status');

        $statuses = ['new', 'assigned', 'investigating', 'legal_action', 'funds_recovered', 'withdrawal_ready', 'closed'];
        $cases_by_status = [];
        foreach ($statuses as $s) {
            $cases_by_status[$s] = (int) ($caseCounts[$s] ?? 0);
        }

        $total_cases = FraudCase::count();
        $unassigned_cases = FraudCase::whereNull('team_member_id')->count();

        // Monetary aggregates
        $total_amount_lost = (float) FraudCase::sum('amount_lost');
        $total_amount_recovered = (float) FraudCase::sum('amount_recovered');
        $recovery_rate = $total_amount_lost > 0
            ? round(($total_amount_recovered / $total_amount_lost) * 100, 1)
            : 0;

        // Fee request stats
        $pending_fees_count = FeeRequest::where('status', 'pending')->count();
        $paid_fees_count = FeeRequest::where('status', 'paid')->count();
        $pending_fees_amount = (float) FeeRequest::where('status', 'pending')->sum('amount');
        $paid_fees_amount = (float) FeeRequest::where('status', 'paid')->sum('amount');
        ...
        $recent_cases = FraudCase::with('user')->orderByDesc('id')->limit(8)->get();
