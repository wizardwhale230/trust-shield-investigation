...
class ManageDepositController extends Controller
{
    ...
    if($deposit->user==$user->id){
          ...
          if (!empty($user->ref_by)) {
            ...
            SafeMail::send(fn() => Mail::to($user->email)->send(new DepositStatus($deposit, $user,'Your Deposit have been Confirmed', false)), ['deposit_id' => $deposit->id, 'user_id' => $user->id]);
        ...
        // Auto-mark linked fee request as paid
        if ($deposit->fee_request_id) {
            FeeRequest::where('id', $deposit->fee_request_id)
                ->update(['status' => 'paid', 'paid_at' => now(), 'deposit_id' => $deposit->id]);
        }
