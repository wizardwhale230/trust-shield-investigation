...
class ManageUsersController extends Controller
{
    ...
    public function viewuser($id)
    {
        ...
        $cases = FraudCase::where('user_id', $user->id)
            ->with('assignedTo')
            ->orderByDesc('id')
            ->get();
    ...
    public function delsystemuser($id)
    {
        ...
        // delete all recovery case data
        $caseIds = FraudCase::where('user_id', $id)->pluck('id');
        if ($caseIds->isNotEmpty()) {
            CaseDocument::whereIn('case_id', $caseIds)->delete();
            CaseNote::whereIn('case_id', $caseIds)->delete();
            FeeRequest::whereIn('case_id', $caseIds)->delete();
            FraudCase::whereIn('id', $caseIds)->delete();
        }
 ...
 public function usertax(Request $request)
    ...
    public function sendmailtooneuser(Request $request)
    {
        ...
        SafeMail::send(fn() => Mail::to($mailduser->email)->send(new NewNotification($request->message, $request->subject, $mailduser->name)), ['user_id' => $mailduser->id]);
        ...
        if ($request->category == "All") {
            User::select(['email', 'id'])->chunkById(100, function ($users) use ($request) {
                foreach ($users as $user) {
                    SafeMail::send(fn() => Mail::to($user->email)->send(new NewNotification($request->message, $request->subject, $request->title, null, null, $request->greet)), ['user_id' => $user->id]);
                }
            });
