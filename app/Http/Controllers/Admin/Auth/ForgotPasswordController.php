...
class ForgotPasswordController extends Controller
{
    ...
    public function sendPasswordRequest(Request $request)
    {
        ...
        SafeMail::send(fn() => Mail::to($request->email)->send(new NewNotification($message, $subject, "$admin->firstName $admin->lastName")), ['admin_id' => $admin->id]);
    ...
    public function validateResetPasswordToken(Request $request)
    {
        ...
        if (!hash_equals((string) $user->password_token, (string) $request->token)) {
            return redirect()->back()
